...
const SearchBar = () => {
  ...
  const { isDark,data,setFilteredData,filteredByRegion }=useContext(context);
  ...
  return (
    <div className={`flex flex-wrap w-full  lg:px-14 md:px-6 md:justify-between md:items-center ${isDark?"bg-[var(--very-dark-blue-bg)]":"bg-[var(--very-light-gray-bg)]"}`} >
      <div className="w-full px-4 py-4 relative max-w-125 lg:px-0">
        ...
        <input type='text' className={`shadow-[0_0_8px_0_rgba(0,0,0,0.2)] w-full h-8  rounded px-7 lg:h-10 m:h-9 ${isDark?"bg-[var(--dark-blue-elements)] text-[var(--white)]":"bg-white"}`} placeholder='Search for a country' onChange={inputHandler}  />
      </div>

      <div className='px-4 lg:pt-4 lg:px-0 m:pt-4.5'>
        <FilterRegion setSelectFilter={setFilterValue} />
      </div>
    </div>
  );
};
